...
import zzzde.n.knowledge.pattern.strategyMe.someWay.BatchSendForFileThread;
...
//    }
    public static void main(String[] args) {
        System.out.println("------------start------------");
        SendQO param = new SendQO();
        param.setContent("线程内容").setName("入参一").setType(1);
        SendThreadWay batchSendForFileThread = new BatchSendForFileThread();
        batchSendForFileThread.setParam(param);
        Thread thread = new Thread(batchSendForFileThread);
        thread.start();
        System.out.println("------------end------------");
    }
